...
export const MainPage: React.FC<{}> = (props) => {
  ...
  const [staticHighlights, setStaticHighlights] =
    useState<StaticHighlightObject>([]);
  ...
  const [AllDFAHighlights, setAllDFAHighlights] = useState<DFAHighlightObject>(
    {}
  ); //this one is accumulating, not refreshing
  const [dagreGraph, setDagreGraph] = useState<DagreGraphObject>({
    nodes: [],
    edges: [],
  });
  ...
  function generateSegments(regex: string) {
    ...
    function matchSegments(idxPair: number[]) {
      const states: DFAStateObject = matchDFAfromSub(
        graph,
        idxs,
        idxPair,
        text
  ...
  return (
    <Container>
      ...
      /> */}
      <DFAVis DagreGraph={dagreGraph} />
      {/* <h1>{DFAActiveState["coin"]}</h1> */}
    </Container>
  );
};
